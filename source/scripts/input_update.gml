...
if (global.infocus) for (i=0;i<vii_sizeof;i+=1) {
    ...
    new_keyboard_check_direct(vii_code[i])
    vii_key[i]=new_keyboard_check_direct(vii_code[i])
    ...
    if (!global.input_cleared) {                            //but ignore shift while doing that
        vii_press[i]=new_keyboard_check_pressed(vii_code[i]) && (vii_prev[i]!=0 || vii_code[i]=vk_shift)
        vii_release[i]=new_keyboard_check_released(vii_code[i])
    }
